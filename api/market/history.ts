...
export default async function handler(req: AnyRequest, res?: AnyResponse) {
  ...
  try {
    ...
    await pool.query(`CREATE SCHEMA IF NOT EXISTS stock_advisor; CREATE TABLE IF NOT EXISTS stock_advisor.sync_runs (id BIGSERIAL PRIMARY KEY, run_key VARCHAR(96) NOT NULL UNIQUE, status VARCHAR(16) NOT NULL, started_at BIGINT NOT NULL, finished_at BIGINT, assets_processed INTEGER NOT NULL DEFAULT 0, assets_succeeded INTEGER NOT NULL DEFAULT 0, error_message TEXT, summary_title TEXT, detail_text TEXT, details_json JSONB, created_at TIMESTAMPTZ NOT NULL DEFAULT now()); ALTER TABLE stock_advisor.sync_runs ADD COLUMN IF NOT EXISTS summary_title TEXT; ALTER TABLE stock_advisor.sync_runs ADD COLUMN IF NOT EXISTS detail_text TEXT; ALTER TABLE stock_advisor.sync_runs ADD COLUMN IF NOT EXISTS details_json JSONB; CREATE TABLE IF NOT EXISTS stock_advisor.email_deliveries (id BIGSERIAL PRIMARY KEY, run_key VARCHAR(96) NOT NULL UNIQUE, recipient VARCHAR(320) NOT NULL, status VARCHAR(16) NOT NULL, provider_message_id VARCHAR(255), error_message TEXT, sent_at BIGINT, created_at TIMESTAMPTZ NOT NULL DEFAULT now()); CREATE TABLE IF NOT EXISTS stock_advisor.sync_run_assets (id BIGSERIAL PRIMARY KEY, run_key VARCHAR(96) NOT NULL, asset_id BIGINT NOT NULL, ticker VARCHAR(32) NOT NULL, display_name VARCHAR(255) NOT NULL, status VARCHAR(16) NOT NULL, previous_price NUMERIC(20,6), price NUMERIC(20,6), bid NUMERIC(20,6), ask NUMERIC(20,6), change_percent NUMERIC(12,6), source_name VARCHAR(128), source_url VARCHAR(1024), as_of BIGINT, message TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(run_key, asset_id)); CREATE TABLE IF NOT EXISTS stock_advisor.ai_advice_runs (id BIGSERIAL PRIMARY KEY, run_key VARCHAR(96) NOT NULL UNIQUE, workspace_key VARCHAR(96) NOT NULL DEFAULT 'owner', requested_ticker VARCHAR(32), additional_requirement TEXT, model VARCHAR(64) NOT NULL, status VARCHAR(16) NOT NULL, assets_requested INTEGER NOT NULL DEFAULT 0, assets_analyzed INTEGER NOT NULL DEFAULT 0, assets_skipped INTEGER NOT NULL DEFAULT 0, error_message TEXT, summary_title TEXT, detail_text TEXT, response_json JSONB, started_at BIGINT NOT NULL, finished_at BIGINT, created_at TIMESTAMPTZ NOT NULL DEFAULT now()); ALTER TABLE stock_advisor.ai_advice_runs ADD COLUMN IF NOT EXISTS summary_title TEXT; ALTER TABLE stock_advisor.ai_advice_runs ADD COLUMN IF NOT EXISTS detail_text TEXT;`);
    const [sync, syncAssets, email, advice] = await Promise.all([
      pool.query(dateRange ? `SELECT run_key AS "runKey", status, started_at AS "startedAt", finished_at AS "finishedAt", assets_processed AS "assetsProcessed", assets_succeeded AS "assetsSucceeded", error_message AS "errorMessage", summary_title AS "summaryTitle", detail_text AS "detailText", details_json AS "detailsJson", created_at AS "createdAt" FROM stock_advisor.sync_runs WHERE started_at BETWEEN $1 AND $2 ORDER BY started_at DESC LIMIT 100` : `SELECT run_key AS "runKey", status, started_at AS "startedAt", finished_at AS "finishedAt", assets_processed AS "assetsProcessed", assets_succeeded AS "assetsSucceeded", error_message AS "errorMessage", summary_title AS "summaryTitle", detail_text AS "detailText", details_json AS "detailsJson", created_at AS "createdAt" FROM stock_advisor.sync_runs ORDER BY started_at DESC LIMIT 30`, dateRange ? [dateRange.start, dateRange.end] : []),
      pool.query(`SELECT run_key AS "runKey", ticker, display_name AS "displayName", status, previous_price AS "previousPrice", price, bid, ask, change_percent AS "changePercent", source_name AS "sourceName", source_url AS "sourceUrl", as_of AS "asOf", message, created_at AS "createdAt" FROM stock_advisor.sync_run_assets ORDER BY created_at DESC LIMIT 500`),
      pool.query(dateRange ? `SELECT run_key AS "runKey", recipient, status, provider_message_id AS "providerMessageId", error_message AS "errorMessage", sent_at AS "sentAt", created_at AS "createdAt" FROM stock_advisor.email_deliveries WHERE sent_at BETWEEN $1 AND $2 ORDER BY created_at DESC LIMIT 100` : `SELECT run_key AS "runKey", recipient, status, provider_message_id AS "providerMessageId", error_message AS "errorMessage", sent_at AS "sentAt", created_at AS "createdAt" FROM stock_advisor.email_deliveries ORDER BY created_at DESC LIMIT 30`, dateRange ? [dateRange.start, dateRange.end] : []),
      pool.query(dateRange ? `SELECT run_key AS "runKey", requested_ticker AS "requestedTicker", additional_requirement AS "additionalRequirement", model, status, assets_requested AS "assetsRequested", assets_analyzed AS "assetsAnalyzed", assets_skipped AS "assetsSkipped", error_message AS "errorMessage", summary_title AS "summaryTitle", detail_text AS "detailText", response_json AS "responseJson", started_at AS "startedAt", finished_at AS "finishedAt", created_at AS "createdAt" FROM stock_advisor.ai_advice_runs WHERE workspace_key='owner' AND started_at BETWEEN $1 AND $2 ORDER BY started_at DESC LIMIT 100` : `SELECT run_key AS "runKey", requested_ticker AS "requestedTicker", additional_requirement AS "additionalRequirement", model, status, assets_requested AS "assetsRequested", assets_analyzed AS "assetsAnalyzed", assets_skipped AS "assetsSkipped", error_message AS "errorMessage", summary_title AS "summaryTitle", detail_text AS "detailText", response_json AS "responseJson", started_at AS "startedAt", finished_at AS "finishedAt", created_at AS "createdAt" FROM stock_advisor.ai_advice_runs WHERE workspace_key='owner' ORDER BY started_at DESC LIMIT 30`, dateRange ? [dateRange.start, dateRange.end] : []),
    ]);
