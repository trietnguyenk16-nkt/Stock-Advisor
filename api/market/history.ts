...
export default async function handler(req: AnyRequest, res?: AnyResponse) {
  ...
  try {
    ...
    const [sync, syncAssets, email, advice] = await Promise.all([
      pool.query(dateRange ? `SELECT run_key AS "runKey", status, started_at AS "startedAt", finished_at AS "finishedAt", assets_processed AS "assetsProcessed", assets_succeeded AS "assetsSucceeded", error_message AS "errorMessage", created_at AS "createdAt" FROM stock_advisor.sync_runs WHERE started_at BETWEEN $1 AND $2 ORDER BY started_at DESC LIMIT 100` : `SELECT run_key AS "runKey", status, started_at AS "startedAt", finished_at AS "finishedAt", assets_processed AS "assetsProcessed", assets_succeeded AS "assetsSucceeded", error_message AS "errorMessage", created_at AS "createdAt" FROM stock_advisor.sync_runs ORDER BY started_at DESC LIMIT 30`, dateRange ? [dateRange.start, dateRange.end] : []),
