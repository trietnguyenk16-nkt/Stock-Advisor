...
export default async function handler(request: Request) {
  ...
  try {
    ...
    return json({ ticker, name: meta.longName ?? meta.shortName ?? ticker, currency: meta.currency ?? "", price, change, asOf: meta.regularMarketTime ? new Date(meta.regularMarketTime * 1000).toISOString() : new Date().toISOString(), source: "Yahoo Finance public chart endpoint" });
  } catch (error) {
