...
import { formatConfidence, formatNumber, formatPercent } from "@/lib/formatters";
...
function formatPrice(asset: Asset) {
  return formatNumber(asset.price, { maximumFractionDigits: 2 });
}

function formatChange(asset: Asset) {
  if (typeof asset.price === "number" && Number.isFinite(asset.price) && (asset.change == null || !Number.isFinite(asset.change))) return "—";
  return formatPercent(asset.change);
}
...
export default function Home() {
  ...
  const [comparisonPrices, setComparisonPrices] = useState<Record<string, { price: number | null; sourceName: string | null }>>({});
  ...
  useEffect(() => {
    if (!showDateComparison) return;
    let active = true;
    directApi.history(selectedDate).then((history) => {
      if (!active) return;
      setComparisonPrices(Object.fromEntries(history.comparisons.map((item) => [item.ticker, { price: item.price, sourceName: item.sourceName }])));
    }).catch(() => { if (active) setComparisonPrices({}); });
    return () => { active = false; };
  }, [selectedDate, showDateComparison]);
  ...
  const startAiAnalysis = async () => {
    ...
    try {
      ...
      setLastAiResult(`${analyzedCount} tài sản đã phân tích${skippedCount ? ` · ${skippedCount} tài sản chờ dữ liệu` : ""}${errorCount ? ` · ${errorCount} lỗi cần kiểm tra` : ""}`);
      if (analyzedCount > 0) toast.success("Đã hoàn tất phân tích AI", { description: `${analyzedCount} tài sản được cập nhật bằng ${modelUsed}.` });
      else toast.error("AI chưa tạo được kết quả", { description: result.errors?.slice(0, 2).join(" | ") || "Không có tài sản nào có quote hợp lệ." });
    } catch (error) {
      ...
      <div className="mx-auto w-full max-w-[1480px] px-4 py-4 sm:px-6 lg:px-10 lg:py-7">
        ...
        <section className="hero-panel mb-6 overflow-hidden rounded-[24px] p-6 sm:p-8 lg:p-10">
          <div className="relative z-10 max-w-3xl">
            ...
            <h1 className="mt-4 max-w-2xl text-3xl font-semibold leading-[1.08] tracking-[-0.045em] text-[#f6faf6] sm:text-5xl">Theo dõi dữ liệu, hiểu rõ quyết định đầu tư.</h1>
        ...
        <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Metric label="Email digest" value="18:00 hằng ngày" suffix="" icon={<Mail size={17} />} accent="gold" compact />
        </div>

        <section className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(330px,0.8fr)]">
          <Card className="dashboard-card overflow-hidden">
            <CardHeader className="flex flex-col gap-4 border-b border-[#e9eeea] px-5 py-5 sm:px-6 xl:flex-row xl:items-center xl:justify-between">
              <div><p className="eyebrow">WATCHLIST</p><CardTitle className="mt-1 text-lg tracking-[-0.03em]">Danh mục quan tâm</CardTitle></div>
              <div className="flex flex-wrap items-center gap-2"><label className="flex items-center gap-2 text-[11px] font-medium text-[#66786d]">So sánh ngày <input type="date" value={selectedDate} onChange={(event) => { setSelectedDate(event.target.value); setShowDateComparison(true); }} className="h-10 rounded-xl border border-[#d8e2db] bg-white px-2 text-xs" /></label>{showDateComparison && <Button onClick={() => { setSelectedDate(vietnamToday()); setShowDateComparison(false); setComparisonPrices({}); }} variant="outline" className="h-10 rounded-xl border-[#d8e2db] px-3 text-xs">Reset</Button>}<Button onClick={() => setIsAdding((value) => !value)} variant="outline" className="min-h-11 rounded-full border-[#d8e2db] bg-white text-[#265d47] hover:bg-[#eff7f1]"><Plus size={16} className="mr-1.5" />Thêm mã</Button></div>
            </CardHeader>
            ...
            <CardContent className="p-0">
              <div className="hidden grid-cols-[1.45fr_0.8fr_0.85fr_0.8fr_0.9fr_0.7fr_28px] gap-3 px-6 py-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9aa69f] xl:grid"><span>Tài sản</span><span>Giá hiện tại</span><span>Mua / Bán</span><span>Biến động</span><span>Nguồn</span>{showDateComparison && <span>Giá {selectedDate}</span>}<span /></div>
              <div className="divide-y divide-[#edf1ed]">
                {assets.map((asset) => <AssetRow key={asset.ticker} asset={asset} refreshKey={quoteVersion} comparison={showDateComparison ? comparisonPrices[asset.ticker] : undefined} onQuote={(ticker, quote) => setAssets((current) => current.map((item) => item.ticker === ticker ? { ...item, name: quote.name, currency: quote.currency || item.currency, price: quote.price, bid: quote.bid, ask: quote.ask, change: quote.change, source: quote.source, sourceUrl: quote.sourceUrl, changeBasis: quote.changeBasis } : item))} onRemove={removeAsset} />)}
          ...
          <div className="space-y-6">
            <Card className="dashboard-card">
              ...
              <CardContent className="px-5 pb-5"><div className="mb-4 rounded-2xl border border-[#e4ece6] bg-[#fbfdfb] p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-[#2b7152]">Model phân tích</p><p className="mt-1 text-[11px] text-[#89968e]">Lựa chọn được lưu cho các lần đồng bộ sau.</p></div><select aria-label="Chọn model AI" value={selectedAiModel} disabled={!aiConfig?.enabled || isSavingAiModel} onChange={(event) => { const model = event.target.value as AiConfig["model"]; void saveAiModel(model); }} className="h-10 rounded-xl border border-[#dce5df] bg-white px-3 text-xs font-medium text-[#315542] outline-none focus:ring-2 focus:ring-[#b9d8c5]"><option value="gpt-4o-mini">gpt-4o-mini</option><option value="gpt-5-mini">gpt-5-mini</option></select></div><p className="mt-2 text-[10px] text-[#9aa59e]">{aiConfigError ? "Không gọi được API cấu hình OpenAI" : !aiConfig ? "Đang kiểm tra cấu hình…" : aiConfig.enabled ? "OpenAI đã sẵn sàng" : "Chưa cấu hình OPENAI_API_KEY trên Vercel"}</p></div><div className="rounded-2xl bg-[#f4f8f4] p-4"><div className="mb-3 flex items-center justify-between"><span className="text-xs font-semibold text-[#2b7152]">{aiConfig?.enabled ? "Sẵn sàng phân tích" : "Cần cấu hình OpenAI"}</span><span className="text-[10px] uppercase tracking-[0.14em] text-[#94a29a]">AI / {assets.length} assets</span></div><label className="mb-3 block text-[11px] font-semibold text-[#52645a]">Yêu cầu<textarea value={aiRequirement} onChange={(event) => setAiRequirement(event.target.value)} placeholder="Ví dụ: đầu tư dài hạn, hoặc chỉ nhập VNM.VN để phân tích riêng mã này" className="mt-1.5 min-h-20 w-full resize-y rounded-xl border border-[#dce5df] bg-white px-3 py-2 text-xs font-normal outline-none focus:ring-2 focus:ring-[#b9d8c5]" /></label><button type="button" onClick={() => void startAiAnalysis()} disabled={isAnalyzingAi || !aiConfig?.enabled} className="mb-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[#1d4734] px-4 py-3 text-xs font-semibold text-white transition hover:bg-[#28624a] disabled:cursor-not-allowed disabled:opacity-50"><Sparkles size={14} />{isAnalyzingAi ? "Đang phân tích…" : "Bắt đầu phân tích AI"}</button>{lastAiResult && <p className="mb-3 text-[11px] font-medium text-[#2b7653]">{lastAiResult}</p>}{false && <div className="mb-4 divide-y divide-[#dbe8de] overflow-hidden rounded-2xl border border-[#dbe8de] bg-white">{aiAnalyses.map((analysis) => <article key={analysis.ticker} className="p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold text-[#244b37]">{analysis.name || analysis.ticker}</p><p className="text-[10px] uppercase tracking-[0.12em] text-[#8b9a90]">{analysis.ticker} · Độ tin cậy {formatConfidence(analysis.confidence)}</p></div><span className={analysis.signal === "BUY" ? "rounded-full bg-[#e4f5e9] px-2.5 py-1 text-[10px] font-bold text-[#23714b]" : analysis.signal === "SELL" ? "rounded-full bg-[#fff0ee] px-2.5 py-1 text-[10px] font-bold text-[#a34f47]" : "rounded-full bg-[#f3f4e9] px-2.5 py-1 text-[10px] font-bold text-[#7c7948]"}>{analysis.signal === "BUY" ? "NÊN MUA" : analysis.signal === "SELL" ? "NÊN BÁN" : "NÊN GIỮ"}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-[#65736b]"><span>Giá tham chiếu <strong className="text-[#315542]">{formatNumber(analysis.referencePrice)}</strong></span><span>Giá mục tiêu <strong className="text-[#315542]">{formatNumber(analysis.targetPrice)}</strong></span></div><p className="mt-3 text-xs leading-5 text-[#52645a]">{analysis.summary}</p><p className="mt-2 text-[11px] leading-5 text-[#8a6652]">Rủi ro: {analysis.risk}</p>{Array.isArray(analysis.news) && analysis.news.length > 0 && <p className="mt-2 text-[10px] text-[#8b9a90]">Nguồn tin: {analysis.news.map((item) => item.publisher).filter(Boolean).join(", ")}</p>}</article>)}</div>}<p className="text-sm leading-6 text-[#65736b]">Nếu không có yêu cầu đặc biệt, AI phân tích mọi mã có giá hiện tại trong Watchlist. Nếu chỉ nhập một ticker đang có trong Watchlist, AI chỉ tư vấn mã đó. Hãy thêm và đồng bộ mã trước khi phân tích.</p><div className="mt-4 flex items-center gap-2 text-[11px] text-[#8b9891]"><FileText size={13} />Phân tích sử dụng giá hiện tại và tin tức được cung cấp theo từng mã</div></div><p className="mt-3 text-[11px] leading-5 text-[#9aa59e]">Phân tích AI chỉ mang tính tham khảo, không phải tư vấn đầu tư được cấp phép.</p></CardContent>
            </Card>
            <AiResultsPanel analyses={aiAnalyses} />
...
function AssetRow({ asset, onRemove, refreshKey, onQuote, comparison }: { asset: Asset; onRemove: (ticker: string) => void; refreshKey: number; onQuote: (ticker: string, quote: Quote) => void; comparison?: { price: number | null; sourceName: string | null } }) {
  ...
  const liveAsset: Asset = quote ? { ...asset, name: quote.name, currency: quote.currency || asset.currency, price: quote.price, bid: quote.bid, ask: quote.ask, change: quote.change, source: quote.source, sourceUrl: quote.sourceUrl, changeBasis: quote.changeBasis } : asset;
  const hasChange = typeof liveAsset.change === "number";
  const comparisonTone = comparison?.price == null || liveAsset.price == null ? "text-[#909d95]" : comparison.price < liveAsset.price ? "text-[#c45e54]" : comparison.price > liveAsset.price ? "text-[#2d8961]" : "text-[#b68c26]";
  return <div className="asset-row relative grid gap-3 px-5 py-4 pr-14 xl:grid-cols-[1.45fr_0.8fr_0.85fr_0.8fr_0.9fr_0.7fr_28px] xl:items-center xl:gap-3 xl:px-6"><div className="flex min-w-0 items-center gap-3"><div className={`ticker-badge ${liveAsset.kind === "Vàng" ? "gold" : liveAsset.kind === "Chứng chỉ quỹ" ? "fund" : "stock"}`}>{liveAsset.ticker.replace(".VN", "").replace("=F", "").slice(0, 4)}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#2c3e32]">{liveAsset.name}</p><p className="mt-0.5 truncate text-[11px] text-[#95a199]">{liveAsset.ticker} · {liveAsset.kind}</p></div></div><div className="flex items-center justify-between xl:block"><span className="text-[10px] uppercase tracking-[0.12em] text-[#a0aaa4] xl:hidden">Giá</span><p className="text-sm font-semibold text-[#304439]">{isLoadingQuote ? "Đang tải" : formatPrice(liveAsset)} <span className="text-[10px] font-medium text-[#9da8a1]">{liveAsset.currency}</span></p></div><div className="flex items-center justify-between xl:block"><span className="text-[10px] uppercase tracking-[0.12em] text-[#a0aaa4] xl:hidden">Mua / Bán</span><p className="text-[11px] font-medium text-[#52645a]">{liveAsset.kind === "Vàng" ? `${formatNumber(liveAsset.bid)} / ${formatNumber(liveAsset.ask)}` : "—"}</p></div><div className="flex items-center justify-between xl:block"><span className="text-[10px] uppercase tracking-[0.12em] text-[#a0aaa4] xl:hidden">Biến động</span><div><p className={`flex items-center gap-1 text-sm font-semibold ${hasChange && liveAsset.change! >= 0 ? "text-[#2d8961]" : hasChange ? "text-[#c45e54]" : "text-[#909d95]"}`}>{hasChange && (liveAsset.change! >= 0 ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />)}{formatChange(liveAsset)}</p><p className="mt-0.5 text-[9px] text-[#9aa69f]">{liveAsset.changeBasis ?? "So với mốc liền trước"}</p></div></div><div className="flex items-center justify-between xl:block"><span className="text-[10px] uppercase tracking-[0.12em] text-[#a0aaa4] xl:hidden">Nguồn</span>{liveAsset.sourceUrl ? <a className="text-[10px] font-medium text-[#2b7653] hover:underline" href={liveAsset.sourceUrl} target="_blank" rel="noreferrer">{liveAsset.source}</a> : <span className="text-[10px] text-[#909d95]">{liveAsset.source ?? "Đang tải"}</span>}</div>{comparison !== undefined && <div className="flex items-center justify-between xl:block"><span className="text-[10px] uppercase tracking-[0.12em] text-[#a0aaa4] xl:hidden">Giá ngày chọn</span><p className={`text-sm font-semibold ${comparisonTone}`}>{comparison.price == null ? "Không có dữ liệu" : formatNumber(comparison.price)}</p><p className="mt-0.5 text-[9px] text-[#9aa69f]">{comparison.sourceName ?? "—"}</p></div>}<button onClick={() => onRemove(asset.ticker)} className="absolute right-4 top-4 grid min-h-11 min-w-11 place-items-center rounded-full text-[#b2bdb6] transition hover:bg-[#f3f7f3] hover:text-[#bd5e54] xl:static xl:min-h-0 xl:min-w-0" aria-label={`Xóa ${asset.ticker}`}><Trash2 size={15} /></button></div>;
}
