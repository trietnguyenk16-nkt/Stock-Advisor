        ...
        {analyses.map((analysis) => (
          <article key={analysis.ticker} className="rounded-2xl border border-[#e1ece3] bg-white p-4 shadow-[0_5px_18px_rgba(35,73,51,0.04)]">
            ...
            <div className="mt-4 space-y-2 text-xs leading-5 text-[#52645a]">
              ...
              <div className="mt-3 rounded-xl border border-[#dcebe0] bg-[#f7fbf7] p-3 text-xs leading-5 text-[#52645a]"><p className="font-bold text-[#315542]">Chiến lược: {analysis.strategy || "Theo dõi thêm dữ liệu trước khi hành động."}</p><div className="mt-2 grid gap-1.5 sm:grid-cols-2"><p><strong>Vùng vào lệnh:</strong> {analysis.entryZone || "—"}</p><p><strong>Tỷ trọng:</strong> {analysis.positionSizing || "—"}</p><p><strong>Chốt lời:</strong> {analysis.takeProfit || "—"}</p><p><strong>Cắt lỗ:</strong> {analysis.stopLoss || "—"}</p></div><p className="mt-2 text-[#7a6657]"><strong>Vô hiệu hóa:</strong> {analysis.invalidation || "—"}</p></div>
            </div>
